...
app.get('/', function(req, res, next) {
    req.getConnection(function(error, conn) {
        conn.query('SELECT * FROM users ORDER BY id DESC',function(err, rows, fields) {
            ...
            if (err) {
                req.flash('error', err)
                res.render('backend/user/list', {
                    title: 'User List', 
                    data: ''
                })
            } else {
                // render to views/backend/user/list.ejs template file
                res.render('backend/user/list', {
                    title: 'User List', 
                    data: rows,
...
app.get('/add', function(req, res, next){    
    // render to views/backend/user/add.ejs
    ...
    req.getConnection(function(error, conn) {
        conn.query('SELECT * FROM users WHERE id = ' + req.params.id, function(err, rows, fields) {
            ...
            if (rows.length <= 0) {
                ...
                // render to views/backend/user/edit.ejs template file
                res.render('backend/user/edit', {
                    title: 'Edit User', 
                    //data: rows[0],
                    id: rows[0].id,
                    name: rows[0].name,
    ...
    req.getConnection(function(error, conn) {
        conn.query('DELETE FROM users WHERE id = ' + req.params.id, user, function(err, result) {
            ...
            if (err) {
                ...
                req.flash('success', 'User '+name+' deleted successfully!')
                // redirect to users list page
                res.redirect('/backend/users')
            }
        })
